...
public:
  ...
  {
    // TODO: this looks VERY sketchy, given that were dealing with a const ref ...
   ...
   */
  template <class CallClientClass>
  std::string createClient(const std::string &rr, const std::string &server)
  {
  ...
  void storeClientQueryStatusCb(const std::string client_id
...
protected:
  ...
  virtual bool callStatusClient(const std::string &target_rr
  ...
  void privateCall(const std::string *rr
  , RrBase *target, const std::string &server
  , QueryType &query
  , const StatusCallType &status_callback)
  { START_SPAN
  ...
  virtual void unloadResource(const std::string &id
...
private:
  ...
  void findAndCollectIdQueries(const std::set<std::string> &ids
  , const std::string &origin_rr
  , std::map<UUID, std::pair<std::string
