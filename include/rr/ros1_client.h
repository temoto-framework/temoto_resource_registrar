...
public:
  ...
  {
    TEMOTO_DEBUG_STREAM_("constructing Ros1Client " << rr << " id: " << name);
 ...
 */
  ...
  {
    TEMOTO_DEBUG_STREAM_("invoke for ServiceClass started. Targeting " << id());
    if (client_.call(request))
    {
      TEMOTO_DEBUG_STREAM_("invoke OK " << request.response.temoto_metadata.request_id);
    }
    else
    {
      TEMOTO_DEBUG_("FAIL");
    }
    TEMOTO_DEBUG_STREAM_("invoke for ServiceClass completed");
  }
 ...
 */
  ...
  {
    TEMOTO_DEBUG_STREAM_("invoke for Ros1Query wrapper started");
    ServiceClass service_call = wrapped_request.rosQuery();

    invoke(service_call);

    wrapped_request = Ros1Query<ServiceClass>(service_call);
    TEMOTO_DEBUG_STREAM_("invoke for Ros1Query wrapper completed");
  }

  void registerUserStatusCb(const std::string &request_id, const UserStatusCb &user_status_cb)
  {
    TEMOTO_DEBUG_STREAM_("registerUserStatusCb "
                    << " - " << id() << " request: " << request_id);

    status_callbacks_[request_id] = user_status_cb;

    TEMOTO_DEBUG_STREAM_("check to see if it really registered: " << hasRegisteredCb(request_id) << " nr of callbacks: " << status_callbacks_.size());
  ...
  {
    TEMOTO_DEBUG_STREAM_("internalStatusCallback"
                    << " - " << id() << " request: " << request_id);

    TEMOTO_DEBUG_STREAM_("Determinging if client cas callback for id " << request_id << " nr of callbacks: " << status_callbacks_.size());
    if (hasRegisteredCb(request_id))
    {
      TEMOTO_DEBUG_STREAM_("!Executing user CB!");
  ...
  {
    TEMOTO_DEBUG_STREAM_("hasRegisteredCb "
                    << " - " << id() << " request: " << request_id);
